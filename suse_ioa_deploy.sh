...
members='[{"ipv4addr": "192.168.124.161", "name": "member1.com"}, {"ipv4addr": "192.168.124.162", "name": "member2.com"}, {"ipv4addr": "192.168.124.163", "name": "member3.com"}]'
if [ x$IP == x ]; then
...
eth_port=`ifconfig -a |grep eth |awk '{print $1}'|head -1`
...
sed -i 's/IOA_SKIP_IPV6_DISABLING=.*/IOA_SKIP_IPV6_DISABLING=false/' $f2
sed -i 's/IOA_SKIP_RELAY=.*/IOA_SKIP_RELAY=false/' $f2
sed -i 's/IOA_SKIP_NETWORK_CONFIGURATION=.*/IOA_SKIP_NETWORK_CONFIGURATION=false/' $f2
sed -i 's/export IOA_DHCP_RELAY_MANAGEMENT_NETWORK=.*/export IOA_DHCP_RELAY_MANAGEMENT_NETWORK=192.168.124.0\/24/' $f2
sed -i 's/export IOA_DHCP_RELAY_MANAGEMENT_NETWORK_VIEW=.*/export IOA_DHCP_RELAY_MANAGEMENT_NETWORK_VIEW=test_mgmt/' $f2
